 ...
 */
export async function markPartialTPLanded(positionKey, attempt = 1) {
 ...
 */
export async function clearPartialTPGuard(positionKey) {
  return withFileLock(STATE_PATH, async () => {
    const state = load();
 ...
 */
export async function prunePartialTPGuard(maxAgeDays = 7) {
  return withFileLock(STATE_PATH, async () => {
    const state = load();
