...
describe("isPartialTPLanded", () => {
  ...
  it("returns true after markPartialTPLanded", async () => {
    let stored = {};
    ...
    await markPartialTPLanded("pos1", 1);
    expect(isPartialTPLanded("pos1", 1)).toBe(true);
  });

  it("different attempt key does not block", async () => {
    let stored = {};
    vi.mocked(fs.readFileSync).mockImplementation(() => JSON.stringify(stored));
    vi.mocked(fs.writeFileSync).mockImplementation((_, data) => { stored = JSON.parse(data); });
...
describe("clearPartialTPGuard", () => {
  it("removes all entries for a position", async () => {
    let stored = {};
    vi.mocked(fs.readFileSync).mockImplementation(() => JSON.stringify(stored));
    vi.mocked(fs.writeFileSync).mockImplementation((_, data) => { stored = JSON.parse(data); });

    await markPartialTPLanded("pos2", 1);
    await markPartialTPLanded("pos2", 2);
    await clearPartialTPGuard("pos2");
...
describe("prunePartialTPGuard", () => {
  it("removes entries older than maxAgeDays", async () => {
    ...
    const pruned = await prunePartialTPGuard(7);
